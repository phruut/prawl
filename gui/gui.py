...
class PrawlGUI:
    def __init__(self, config, timer, keyseq, state, update):
        ...
        dpg.create_context()
        create_themes()
        self.main_font, self.icon_font = create_fonts(config.main_font, config.icon_font)
            ...
            with dpg.group(tag='settings_group', show=False):
                ...
                with dpg.collapsing_header(label='general', bullet=True):
                    ...
                    with dpg.tree_node(label='starting / restarting'):
                        dpg.add_spacer(height=0.5)
                        dpg.add_text('spam amount'); dpg.add_slider_int(label='presses', min_value=0, max_value=20, default_value=int(self.config.get('game_start_spam', 12)), tag='start_spam')
                        with dpg.tooltip(dpg.last_item()): dpg.add_text('how many times to press going through match result screen etc', wrap=190)
                        dpg.add_text('match restart delay'); dpg.add_slider_int(label='seconds', min_value=0, max_value=30, default_value=int(self.config.get('game_restart_delay', 4)), tag='wait_restart')
                        with dpg.tooltip(dpg.last_item()): dpg.add_text('the time to wait after a match finishes before restarting', wrap=190)
                        dpg.add_text('match load time'); dpg.add_slider_int(label='seconds', min_value=10, max_value=30, default_value=int(self.config.get('game_load_time', 15)), tag='wait_gameload')
                    ...
                    with dpg.tree_node(label='disconnect / reconnect'):
                        ...
                        dpg.add_checkbox(label='default', tag='open_menu_default', default_value=bool(self.config.get('open_menu_default', True)), callback=self.callbacks.select_open_menu_default)
                        with dpg.tooltip(dpg.last_item()): dpg.add_text('default mode c:', wrap=190)
                        dpg.add_spacer(height=0.5)
                        with dpg.group(horizontal=True):
                            dpg.add_checkbox(label='+fix', tag='open_menu_fix', default_value=bool(self.config.get('open_menu_fix', False)), callback=self.callbacks.select_open_menu_fix)
                            ...
                            dpg.add_checkbox(label='+fix2', tag='open_menu_fix2', default_value=bool(self.config.get('open_menu_fix2', False)), callback=self.callbacks.select_open_menu_fix2)
                            with dpg.tooltip(dpg.last_item()): dpg.add_text('tries the fix but 2 times', wrap=190)
                        dpg.add_spacer(height=0.5)
                        dpg.add_checkbox(label='hold to pause', tag='open_menu_hold', default_value=bool(self.config.get('open_menu_hold', False)), callback=self.callbacks.select_open_menu_hold)
                        with dpg.tooltip(dpg.last_item()): dpg.add_text('must enable in brawlhalla: OPTIONS > SYSTEM SETTINGS > HOLD TO PAUSE', wrap=190)
                        dpg.add_spacer(height=0.5)
                        dpg.add_checkbox(label='use ENTER key', tag='open_menu_enter', default_value=bool(self.config.get('open_menu_enter', True)))
                        with dpg.tooltip(dpg.last_item()): dpg.add_text('use ENTER key instead of ESC', wrap=190)
                        dpg.add_spacer(height=0.5)
                        dpg.add_text('menu key presses', tag='menu_key_presses_text'); dpg.add_slider_int(label='times', min_value=1, max_value=4, default_value=int(self.config.get('menu_key_presses', 2)), tag='menu_key_presses')
                        ...
                        dpg.add_text('disconnect delay'); dpg.add_slider_int(label='ms', min_value=100, max_value=1000, default_value=int(self.config.get('disconnect_delay', 120)), tag='wait_disconnect')
                        with dpg.tooltip(dpg.last_item()): dpg.add_text('time to wait for the menu to pop up after pressing menu key', wrap=190)
                        dpg.add_text('reconnect delay'); dpg.add_slider_int(label='seconds', min_value=3, max_value=10, default_value=int(self.config.get('reconnect_delay', 4)), tag='wait_reconnect')
                    ...
                    with dpg.tree_node(label='input config'):
                        dpg.add_spacer(height=0.5)
                        dpg.add_checkbox(label='direct input mode', tag='direct_input', default_value=bool(self.config.get('direct_input', False)))
                        ...
                        with dpg.group(horizontal=True):
                            ...
                            dpg.add_button(label=self.config.get('key_up', 'up'), tag='key_up_button', width=26, height=26, callback=self.callbacks.hotkey_button, user_data='key_up')
                            with dpg.tooltip(dpg.last_item()): dpg.add_text('change up key', tag='key_up_tooltip_text')
                        dpg.add_spacer(height=0.5)
                        with dpg.group(horizontal=True):
                            dpg.add_button(label=self.config.get('key_left', 'left'), tag='key_left_button', width=26, height=26, callback=self.callbacks.hotkey_button, user_data='key_left')
                            with dpg.tooltip(dpg.last_item()): dpg.add_text('change left key', tag='key_left_tooltip_text')
                            dpg.add_button(label=self.config.get('key_down', 'down'), tag='key_down_button', width=26, height=26, callback=self.callbacks.hotkey_button, user_data='key_down')
                            with dpg.tooltip(dpg.last_item()): dpg.add_text('change down key', tag='key_down_tooltip_text')
                            dpg.add_button(label=self.config.get('key_right', 'right'), tag='key_right_button', width=26, height=26, callback=self.callbacks.hotkey_button, user_data='key_right')
                            with dpg.tooltip(dpg.last_item()): dpg.add_text('change right key', tag='key_right_tooltip_text')
                            dpg.add_spacer(width=13)
                            dpg.add_button(label=self.config.get('key_throw', 'v'), tag='key_throw_button', width=26, height=26, callback=self.callbacks.hotkey_button, user_data='key_throw')
                            with dpg.tooltip(dpg.last_item()): dpg.add_text('change throw weapon key', tag='key_throw_tooltip_text')
                            dpg.add_button(label=self.config.get('key_light', 'c'), tag='key_light_button', width=26, height=26, callback=self.callbacks.hotkey_button, user_data='key_light')
                            with dpg.tooltip(dpg.last_item()): dpg.add_text('change light attack key', tag='key_light_tooltip_text')
                            dpg.add_button(label=self.config.get('key_heavy', 'x'), tag='key_heavy_button', width=26, height=26, callback=self.callbacks.hotkey_button, user_data='key_heavy')
                            ...
                            dpg.add_text(default_value=self.config.get('key_up', 'up'), tag='key_up', show=False)
                            dpg.add_text(default_value=self.config.get('key_left', 'left'), tag='key_left', show=False)
                            dpg.add_text(default_value=self.config.get('key_down', 'down'), tag='key_down', show=False)
                            dpg.add_text(default_value=self.config.get('key_right', 'right'), tag='key_right', show=False)
                            dpg.add_text(default_value=self.config.get('key_throw', 'v'), tag='key_throw', show=False)
                            dpg.add_text(default_value=self.config.get('key_light', 'c'), tag='key_light', show=False)
                            dpg.add_text(default_value=self.config.get('key_heavy', 'x'), tag='key_heavy', show=False)

                        dpg.add_spacer(height=0.5)
                        dpg.add_text('hold'); dpg.add_slider_int(label='ms', min_value=0, max_value=300, default_value=int(self.config.get('keypress_hold', 70)), tag='keypress_hold')
                        with dpg.tooltip(dpg.last_item()): dpg.add_text('average hold duration of the key', wrap=190)
                        dpg.add_text('delay'); dpg.add_slider_int(label='ms', min_value=50, max_value=500, default_value=int(self.config.get('keypress_delay', 150)), tag='keypress_delay')
                ...
                with dpg.collapsing_header(label='boop beep?', bullet=True):
                    dpg.add_spacer(height=0.5)
                    with dpg.group(horizontal=True):
                        dpg.add_checkbox(label='timer sound', tag='timer_sound', default_value=bool(self.config.get('timer_sound', False)))
                    ...
                    dpg.add_text('beep frequency'); dpg.add_slider_int(label='hz', min_value=100, max_value=2000, default_value=int(self.config.get('beep_frequency', 500)), tag='beep_frequency')
                    dpg.add_text('beep duration'); dpg.add_slider_int(label='ms', min_value=10, max_value=1000, default_value=int(self.config.get('beep_duration', 72)), tag='beep_duration')
                    dpg.add_spacer(height=0.5)

                # other things
                with dpg.collapsing_header(label='other', bullet=True):
                    dpg.add_spacer(height=0.5)
                    dpg.add_checkbox(label='always on top', tag='always_on_top', default_value=bool(self.config.get('always_on_top', True)), callback=self.callbacks.update_aot)
                    with dpg.tooltip(dpg.last_item()): dpg.add_text('makes this window stay on top')
                    dpg.add_spacer(height=0.5)
                    dpg.add_checkbox(label='launch brawlhalla with prawl', tag='auto_launch', default_value=bool(self.config.get('auto_launch', False)))
                    with dpg.tooltip(dpg.last_item()): dpg.add_text('launches brawlhalla when you launch prawl')
                    dpg.add_spacer(height=0.5)
                    dpg.add_checkbox(label='rate limit detection', tag='rate_limit_detect', default_value=bool(self.config.get('rate_limit_detect', True)))
                    with dpg.tooltip(dpg.last_item()): dpg.add_text('detects if you are rate limited in exp/gold')
                    dpg.add_spacer(height=0.5)
                    dpg.add_checkbox(label='rate limit auto wait', tag='rate_limit_wait', default_value=bool(self.config.get('rate_limit_wait', True)))
                    with dpg.tooltip(dpg.last_item()): dpg.add_text('waits for rate limit reset and starts farming')
                    dpg.add_spacer(height=0.5)
                    dpg.add_slider_int(label='mins', min_value=30, max_value=60, default_value=int(self.config.get('rate_limit_wait_time', 45)), tag='rate_limit_wait_time')
                    with dpg.tooltip(dpg.last_item()): dpg.add_text('time to wait for rate limit to reset')
                    dpg.add_spacer(height=0.5)
                    dpg.add_checkbox(label='max games', tag='max_games', default_value=bool(self.config.get('max_games', False)))
                    with dpg.tooltip(dpg.last_item()): dpg.add_text('stops after set amount of games')
                    dpg.add_spacer(height=0.5)
                    dpg.add_slider_int(label='games', min_value=1, max_value=99, default_value=int(self.config.get('max_games_amount', 16)), tag='max_games_amount')
